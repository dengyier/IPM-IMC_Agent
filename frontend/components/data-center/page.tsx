...
export function DataCenterPage() {
  ...
  const [tab, setTab] = useState<LayerTab>("all");
  ...
  async function runAction(row: MaterialRow) {
    ...
    try {
      if (row.layer === "expansion") {
        ...
        setToast(
          `已吸收：${res.chunk_count} 切块 → ${res.item_count} 条目 / ${res.review_task_count} 审核任务`
        );
  ...
  return (
    <main className="flex min-w-0 flex-1 overflow-hidden">
      <section className="flex min-w-0 flex-1 flex-col overflow-y-auto px-8 py-6">
        <DataHeader query={query} onQueryChange={setQuery} onRefresh={load} />
        <div className="mt-7 grid gap-5 xl:grid-cols-[1fr_1fr]">
          <UploadCard onUploaded={load} onToast={setToast} />
          <ProcessFlow />
        </div>
        {toast && (
...
function MaterialOverview({
  ...
  stats: { total: number; done: number; processing: number; uploaded: number; failed: number };
}) {
  const items = [
