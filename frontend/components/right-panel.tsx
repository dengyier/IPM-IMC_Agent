    ...
    <Card className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-t border-line/70 bg-white px-6 py-5">
        <form
          ...
          className="flex items-center gap-3 rounded-2xl border border-line bg-white px-4 py-3 shadow-[0_12px_34px_rgba(30,58,138,0.08)] focus-within:border-brand/50"
        >
          <input
            value={input}
            onChange={(event) => setInput(event.target.value)}
            className="min-w-0 flex-1 bg-transparent text-[14px] text-ink outline-none placeholder:text-slate-400"
            placeholder="继续提问..."
          />
          <button
            disabled={loading}
            className="brand-gradient flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-white shadow-soft disabled:opacity-60"
          >
            <Icon name={loading ? "refresh" : "send"} className="h-5 w-5" />
          </button>
        </form>
      </div>
    </Card>
      ...
      <aside className="flex w-[268px] shrink-0 flex-col border-r border-[#ececf1] bg-[#f7f7f8]">
        ...
        <div className="mt-auto border-t border-[#ececf1] px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[13px] font-bold">张晓明</div>
              <div className="text-[11px] text-slate-500">管理员</div>
            </div>
          </div>
        </div>
      </aside>
        ...
        <section className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="px-8 pb-8">
            <form
              ...
              className="mx-auto flex max-w-[840px] items-center gap-3 rounded-[22px] border border-line bg-white px-5 py-3 shadow-[0_18px_60px_rgba(15,23,42,0.11)] focus-within:border-brand/50"
            >
              <button
                type="button"
                onClick={() => createConversation().catch(() => undefined)}
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-slate-500 hover:bg-slate-50 hover:text-brand"
                title="开启新会话"
              >
              ...
              <button
                disabled={loading}
                className="brand-gradient flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-white shadow-soft disabled:opacity-60"
              >
                <Icon name={loading ? "refresh" : "send"} className="h-5 w-5" />
              </button>
            </form>
