# 天机AI 商业决策智能体 · 部署文档（Docker Compose 一键部署）
...
- 后端表结构变化由 `init_db()` 在启动时自动处理：新增表由 `create_all` 建好；给【已存在的表】补新增列对 **SQLite 与 PostgreSQL 均自愈**（幂等 `ADD COLUMN IF NOT EXISTS`），升级老库无需手动改表。
