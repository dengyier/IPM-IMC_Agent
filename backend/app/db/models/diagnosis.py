...
class DiagnosisReport(Base):
    ...
    decision_frame: Mapped[dict] = mapped_column(JsonType, default=dict)
    decision_roles: Mapped[list] = mapped_column(JsonType, default=list)
    scenario_paths: Mapped[list] = mapped_column(JsonType, default=list)
    causal_chains: Mapped[list] = mapped_column(JsonType, default=list)
    ...
    tianji_risk_audit: Mapped[list] = mapped_column(JsonType, default=list)
    ...
    assumption_status: Mapped[list] = mapped_column(JsonType, default=list)
    roles_degraded: Mapped[bool] = mapped_column(Boolean, default=False)
    role_similarity_max: Mapped[float] = mapped_column(Float, default=0.0)
    debate_rounds: Mapped[list] = mapped_column(JsonType, default=list)
    consensus: Mapped[list] = mapped_column(JsonType, default=list)
    disagreements: Mapped[list] = mapped_column(JsonType, default=list)
    ...
    archive_candidates: Mapped[list] = mapped_column(JsonType, default=list)
    algorithm_version: Mapped[str | None] = mapped_column(String(60))
    # 推演资产沉淀后的 ExpansionSource.id（幂等标记 + 前端状态展示）
    tianji_deposited_source_id: Mapped[str | None] = mapped_column(String(36))
